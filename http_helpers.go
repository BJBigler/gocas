...
func RedirectToLogin(w http.ResponseWriter, r *http.Request, referer *url.URL, c *Client) {
...
func setAuthenticationResponse(r *http.Request, a *AuthenticationResponse) {
	if glog.V(2) {
		glog.Infof("Inserting authentication response \n\t%v into current context with key \n\t%s", a, authenticationResponseKey)
	}
